...
//    }
    ...
    public static List<Info> getInfo(String[] titles, String[] contents, String[] times) {
        List<Info> datas = new ArrayList<>();
        for(int i=0; i<titles.length; i++) {
            Info info = new Info("1", titles[i], contents[i], times[i]);
            datas.add(info);
        }
        return datas;
    }

    public static List<Community> getCommunity(String[] titles, String[] contents, String[] times, String tags[], String categorys[]) {
        List<Community> datas = new ArrayList<>();
        for(int i=0; i<titles.length; i++) {
            Community community = new Community("1", titles[i], contents[i], times[i], tags[i], categorys[i]);
            datas.add(community);
        }
        return datas;
    }
